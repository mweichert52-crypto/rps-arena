...
function finishRound(room) {
  ...
  if (a.score >= room.targetWins || b.score >= room.targetWins) {
    ...
    room.finishTimer = setInterval(() => {
      secondsLeft--;
      if (secondsLeft > 0) {
        io.to(room.code).emit('match-countdown', { seconds: secondsLeft });
        return;
      }

      clearInterval(room.finishTimer);
...
io.on('connection', (socket) => {
  socket.on('create-room', ({ name, targetWins }, ack) => {
    ...
    const room = {
      code: makeCode(),
      targetWins: cleanTarget(targetWins),
      players: [{ id: socket.id, name, choice: null, ready: false, score: 0 }, null],
      spectators: new Set(),
      phase: 'waiting',
      countdown: null,
      finishTimer: null
  ...
  socket.on('join-room', ({ code, name }, ack) => {
    ...
    ack({ ok: true, code, role: 'player', targetWins: room.targetWins });
    room.phase = 'playing';
  ...
  socket.on('watch-room', ({ code }, ack) => {
    ...
    ack({ ok: true, code, role: 'spectator', targetWins: room.targetWins });
    socket.emit('spectator-state', publicRoom(room, 'Du schaust anonym zu.'));
    broadcastState(room);
  });
  ...
  socket.on('ready', () => {
    ...
    const player = room.players[role];
    if (!player || !player.choice || player.ready) return;
    ...
    if (room.players.every((p) => p && p.ready)) {
      ...
      const timer = setInterval(() => {
        left--;
        room.countdown = left;
        if (left > 0) io.to(room.code).emit('countdown', { seconds: left });
        else {
          clearInterval(timer);
          room.countdown = null;
          finishRound(room);
