...
document.querySelectorAll('[data-choice]').forEach(b=>b.onclick=()=>{if(role!=='player')return;selected=b.dataset.choice;document.querySelectorAll('[data-choice]').forEach(x=>x.classList.toggle('selected',x===b));});
$('ready-btn').onclick=()=>{if(selected)socket.emit('ready');};
